...
void render_frame(scene* d_scene, cudaGraphicsResource* pbo_resource, cudaTextureObject_t env_tex, int render_mode_frame_count);
...
__global__ void render_scene(uchar4* out, scene* s, cudaTextureObject_t env_tex, int render_mode_frame_count);

/// Trace a ray from camera center through the viewport location (vp_x, vp_y).
__device__ color3 trace_ray(float vp_x, float vp_y, scene* s, cudaTextureObject_t env_tex, unsigned int* seed);
